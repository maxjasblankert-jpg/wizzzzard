/* Wizard — suit-themed card faces (escalating emoji per rank 2–14) */
(function (global) {
  const SUIT_THEMES = {
  ...
  /** Fallback center icon when rank is out of range */
  ...
  /**
   * Escalating themed emoji per rank (index 0 = card value 2, index 12 = value 14).
   * Low ranks = subtle; high ranks = dramatic (e.g. water 2 = droplet → 14 = tsunami wave).
   */
  ...
  /** Map card value 2–14 to themed emoji index (0–12). */
  function getThemeEmojiIndex(value) {
    if (value < 2 || value > 14) return -1;
    return value - 2;
  }

  /** Center emoji: unique per suit + rank; wild cards keep jester/wizard icons. */
  function getRankSymbol(suit, value, card) {
    const v = card?.value ?? value;
    const s = normalizeSuit(card?.suit ?? suit);

    if (v === 1 || (card && isJesterCard(card))) return '🃏';
    if (v === 15 || (card && isWizardCard(card))) return '🧙';

    const idx = getThemeEmojiIndex(v);
    const pool = SUIT_THEME_EMOJIS[s];
    if (idx >= 0 && pool && pool[idx]) return pool[idx];

    return SUIT_PRIMARY_EMOJI[s] || '◆';
  }

  function getSuitEmoji(suit, value, card) {
    return getRankSymbol(suit, value, card);
  }
  ...
  function renderCardInnerHTML(card, options = {}) {
    ...
    const symbol = getRankSymbol(suit, value, card);
    ...
    const rankScaleClass = (value >= 2 && value <= 14) ? ` card-rank-scale-${value}` : '';
    ...
    return `
      ...
      <div class="card-center-icon card-suit-emoji${hideEmoji ? ' card-center-icon-hidden' : ''}${wildClass}${rankScaleClass}" aria-hidden="true">${symbol}</div>
